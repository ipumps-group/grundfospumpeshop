...
const ids = ['1633167548143021', '1612801183557055', '1750963402781157', '966761009025103']
async function api(p) { const r = await fetch(`${BASE}/${p}&access_token=${TOKEN}`); return r.json() }
for (const id of ids) {
  const d = await api(`${id}?fields=name,object_story_spec{link_data{link,image_hash,call_to_action}}`)
  const ld = d.object_story_spec?.link_data || {}
  console.log(`${id}: link=${ld.link} hash=${ld.image_hash} cta=${ld.call_to_action?.type}`)
}
